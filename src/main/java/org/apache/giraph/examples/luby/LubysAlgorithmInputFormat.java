...
import org.apache.giraph.examples.adsWeighted.FacilityLocationADSWeightedVertexValue;
...
public class LubysAlgorithmInputFormat extends TextVertexInputFormat<LongWritable, LubysAlgorithmVertexValue, FloatWritable> {

	@Override
	public TextVertexReader createVertexReader(
			InputSplit split, TaskAttemptContext context) throws IOException {
		return new LubysAlgorithmVertexReaderFromEachLine();
	}
	...
	*/
	...
	private class LubysAlgorithmVertexReaderFromEachLine extends TextVertexReaderFromEachLine {
		...
		@Override
		protected LubysAlgorithmVertexValue getValue(Text line) throws IOException {
			LubysAlgorithmVertexValue value = new LubysAlgorithmVertexValue();
			String[] splitLine = line.toString().split("\t");
			String vertexHashStr = splitLine[1];
			double vertexHash = Double.parseDouble(vertexHashStr);
			value.setVertexValue(vertexHash);
